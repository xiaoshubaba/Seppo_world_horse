...
GR[[i]] = ggscatter(PCplot,size=3,"PC1","PC2",shape="country",color="status",ggtheme=theme_minimal(),alpha=I(0.6),xlab=paste0("PC1(",PC_CONTU[1],"%)"),title=paste0(disList$DistanceName[i]),ylab=paste0("PC2(",PC_CONTU[2],"%)")) + theme(legend.position="right")
...
ggarrange(GR[[1]],GR[[2]],GR[[3]],GR[[4]],GR[[5]],common.legend=TRUE,legend="bottom",labels=LETTERS[1:5])
...
save.image(paste0(args[3],".Rimage"))
